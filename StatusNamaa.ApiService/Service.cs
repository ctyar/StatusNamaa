...
public class Service
{
    ...
    {
        var collector = new MetricCollector<int>(_meterFactory, "OpenTelemetry.Instrumentation.Runtime", "process.runtime.dotnet.monitor.lock_contention.count");
        ...
        while (true)
        {
            ...
            {
                Console.WriteLine($"Value: {measurement.Value}, Timestamp: {measurement.Timestamp}, Tags: {string.Join(", ", measurement.Tags.Select(kvp => $"{kvp.Key}={kvp.Value}"))}");
            }
    ...
    where T : struct
{
    ...
    public MetricCollector(object? meterScope, string meterName, string instrumentName, TimeProvider? timeProvider = null)
        : this(timeProvider)
    {
        Initialize(instrument => true);// Equals(instrument.Meter.Scope, meterScope) && instrument.Meter.Name == meterName && instrument.Name == instrumentName);
    }
    ...
    where T : struct
{
    ...
    //public bool MatchesTags(params string[] tags) => ContainsTags(Tags) && (Tags.Count == tags.Length);
